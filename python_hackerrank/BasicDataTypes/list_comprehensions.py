...
cuboid along with an integer N. You have to print a list of all possible coordinates given by (i, j, k)  on a 3D grid w
here the sum of i+j+k is not equal to N. Here, 0 <= i <= X; 0 <= j <= Y; 0 <= k <= Z
...
if __name__ == '__main__':
    ...
    z = int(raw_input())
    n = int(raw_input())
    l = [ [k,j,i] for k in xrange(x+1) for j in xrange(y+1) for i in xrange(z+1) if k+j+i != n]
    print l