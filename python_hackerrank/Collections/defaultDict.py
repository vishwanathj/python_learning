...
It's similar to the usual dictionary (dict) container, but it has one difference: The value fields' data type is
...
print "\n".join(map(str,([x for x in ([' '.join(map(str, d[raw_input()])) or -1 for i in range(m)])])))